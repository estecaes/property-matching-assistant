...
1. **Duration Validation Edge Case Initially Missed**
...
3. **Budget Extraction Strategy Evolution**
...
1. **Edge Case Discovery Through TDD**: Writing tests first revealed the duration validation edge case that wouldn't have been caught until production. The 0ms scenario is rare but real in fast test environments.

2. **Defensive Strategy Evolution**: Initial implementation followed guidance literally (first-match), but real-world manipulation example ("5 millones pero realmente 3") required evolution to last-match strategy. This is why the anti-injection module is marked as CRITICAL.

3. **Threshold Calibration Matters**: The >50% threshold from guidance was too lenient for financial data. A 40% difference between 5M and 3M should absolutely trigger human review. Adjusted to >30% based on domain context.

4. **Multi-Pattern Extraction Robustness**: Single regex pattern vulnerable to format variations. Using 3 patterns with scan() + validation provides defense-in-depth:
...
5. **Observable Evidence Critical**: The discrepancies array provides clear evidence of potential manipulation. In budget_mismatch scenario, seeing `{field: "budget", llm_value: 5000000, heuristic_value: 3000000, diff_pct: 40, severity: "high"}` makes the issue immediately visible.
...
Module 4 complete with all success criteria verified. Ready to proceed with Module 5 (Property Matching) which will build on the lead_profile data extracted here.
